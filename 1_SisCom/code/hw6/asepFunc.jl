...
function propagation(Nb,η,Np,α,β,ϕ,dx,dt,Nt,ne,s)
...
if s == 1
    save(File(format"JLD",string(path,"/state",ne,"_0.jld")),"σ",σ);
end
...
if s == 1
    # Save the new state
    save(File(format"JLD",string(path,"/state",ne,"_",it,".jld")),"σ",σ);
end

end

# Save the currents
save(File(format"JLD",string(path,"/j_p",ne,".jld")),"j_p",j_p);
save(File(format"JLD",string(path,"/j_n",ne,".jld")),"j_n",j_n);
