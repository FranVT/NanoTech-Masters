...
HPPg = plot(
    ...
    title = L"\mathrm{Histograms~of~sucess~at~different~times}",
    xlabel = L"\mathrm{Number~of~sucesses}",
...
savefig(HPPg,"poissonProcessHistograms.pdf")
...
savefig(μσplot,"poissonProcessAvergeVariance.pdf")
...
map(s->histogram!(HPPg,info[s],label=latexstring("t=$(times[s])"),normalize=:probability),1:length(times))
display(HPPg)
