...
\begin{sol}{Pauli spin matrices properties}{label-2b}
    ...
    \begin{align*}
        \sigma_x\sigma_y  &= \mqty(\pmat{x})\mqty(\pmat{y}) = \mqty(\pmat{z}), \\
        \sigma_y\sigma_z  &= \mqty(\pmat{y})\mqty(\pmat{z}) = \mqty(\pmat{x}), \\
        \sigma_x\sigma_z  &= \mqty(\pmat{x})\mqty(\pmat{z}) = \mqty(\pmat{y}).
    \end{align*}
