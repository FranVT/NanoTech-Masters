...
\begin{sol}{Checking consistency with the uncertanty principle.}{label-3d}
    ...
    \begin{empheq}[box=\shadowbox]{equation*}
        \sigma_{\hat{S}_x}\sigma_{\hat{S}_y}\geq\frac{1}{2}\abs{i\hbar\expval{\hat{S}_z}},\quad 
        \sigma_{\hat{S}_y}\sigma_{\hat{S}_z}\geq\frac{1}{2}\abs{i\hbar\expval{\hat{S}_x}},\quad 
        \sigma_{\hat{S}_x}\sigma_{\hat{S}_z}\geq\frac{1}{2}\abs{i\hbar\expval{\hat{S}_y}}
    \end{empheq}
\end{sol}
...
\begin{sol}{Probability of measuring spin angular momentum along the $x$ direction.}{label-4}
    ...
    \begin{empheq}[box=\shadowbox]{equation*}
        \abs{\braket{\chi_+}{\chi\qty(t)}}^2 =\frac{1}{2}\qty[1+\sin\qty[\alpha]\cos\qty[\gamma B_0t]]
    \end{empheq}
