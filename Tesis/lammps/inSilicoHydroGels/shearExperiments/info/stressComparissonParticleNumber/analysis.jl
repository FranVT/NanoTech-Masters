...
# Get information
shearFiles_names = ("stressVirial_shear_2000.fixf","stressVirial_shear_4000.fixf","stressVirial_shear_6000.fixf");
...
labels_stress = (L"2000 \mathrm{Particles}",L"4000 \mathrm{Particles}",L"6000 \mathrm{Particles}");
...
## Antoher graph
## Prepare labels and stuff
labels_stress2 = (L"T_xx",L"T_xy",L"\mathrm{Norm}");

particles2 = mapreduce(s->s[1,:],hcat,(data_T_xx,data_T_xy,data_norm))';
particles4 = mapreduce(s->s[2,:],hcat,(data_T_xx,data_T_xy,data_norm))';
particles6 = mapreduce(s->s[3,:],hcat,(data_T_xx,data_T_xy,data_norm))';

### Figures
fig_Stress2 = Figure(size=(1200,920));
ax_s12 = Axis(fig_Stress2[1,1:2],
        aspect = nothing,
        title = L"2000~\mathrm{Particles}",
        xlabel = L"\mathrm{Tilt~deformation}",
        ylabel = L"\sigma_{nn}",
        titlesize = 24.0f0,
        xticklabelsize = 18.0f0,
...
ax_s22 = Axis(fig_Stress2[2,1:2],
        aspect = nothing,
        title = L"4000~\mathrm{Particles}",
        xlabel = L"\mathrm{Tilt~deformation}",
        ylabel = L"\sigma_{nn}",
        titlesize = 24.0f0,
        xticklabelsize = 18.0f0,
...
ax_s32 = Axis(fig_Stress2[3,1:2],
        aspect = nothing,
        title = L"6000~\mathrm{Particles}",
        xlabel = L"\mathrm{Tilt~deformation}",
        ylabel = L"\sigma_{nn}",
        titlesize = 24.0f0,
        xticklabelsize = 18.0f0,
    ...
    series!(ax_s12,strain,particles2,labels=labels_stress2,color=:tab10)
    axislegend(ax_s1,"Legends",position=:lb)

    series!(ax_s22,strain,particles4,labels=labels_stress2,color=:tab10)
    axislegend(ax_s2,"Legends",position=:lb)

    series!(ax_s32,strain,particles6,labels=labels_stress2,color=:tab10)
    axislegend(ax_s3,"Legends",position=:lb)

fig_Stress2

save("Comparisson2.png",fig_Stress2)
