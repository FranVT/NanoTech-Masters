"""
    Script to graph the averages of N experiments.
    1 system 
"""
...
# Retrieve the average data
#data=getAvgData(dirs,file_name,parameters);

# Separate the data from assembly and shear experiment
data_assembly=data[1];#first.(data);
data_shear=data[2];#last.(data);
...
time_assembly=range(0,parameters[1][9].*parameters[1][10],length=length(data_assembly[1]));  #Int64(parameters[1][10]/10));

# Auxiliar
tshear_aux=Int64(parameters[1][20]*parameters[1][17]+sum(parameters[1][21:24]));
time_shear=range(0,parameters[1][15]*tshear_aux,length=length(data_shear[1])); # Just the deformation process
...
series!(ax_t,time_assembly,data_assembly[1]',color=csh)
series!(ax_t,time_deform,data_shear[1]',color=csh)
vlines!(ax_t,last(time_assembly).+[time_rlxo1,time_rlxf1,time_rlxo2,time_rlxf2,time_rlxo3,time_rlxf3,time_rlxo4,time_rlxf4],linestyle=:dash,color=:red)
vlines!(ax_t,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_tcp,time_assembly,data_assembly[2]',color=csh)
series!(ax_tcp,time_deform,data_shear[2]',color=csh)
vlines!(ax_tcp,last(time_assembly).+[time_rlxo1,time_rlxf1,time_rlxo2,time_rlxf2,time_rlxo3,time_rlxf3,time_rlxo4,time_rlxf4],linestyle=:dash,color=:red)
...
series!(ax_E,time_assembly,data_assembly[5]',color=csh)
series!(ax_E,time_deform,data_shear[5]',color=csh)
vlines!(ax_E,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_Elo,eachindex(time_assembly),data_assembly[5]',color=csh)
series!(ax_Elo,eachindex(time_deform).+length(time_assembly),data_shear[5]',color=csh)
...
series!(ax_Ut,time_assembly,data_assembly[3]',color=csh)
series!(ax_Ut,time_deform,data_shear[3]',color=csh)
vlines!(ax_Ut,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_wca,time_assembly,data_assembly[6]',color=csh)
series!(ax_wca,time_deform,data_shear[6]',color=csh)
vlines!(ax_wca,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_patch,time_assembly,data_assembly[7]',color=csh)
series!(ax_patch,time_deform,data_shear[7]',color=csh)
vlines!(ax_patch,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_swap,time_assembly,data_assembly[8]',color=csh)
series!(ax_swap,time_deform,data_shear[8]',color=csh)
...
series!(ax_stressXX,data_shear[11]',color=csh)
...
series!(ax_stressXY,data_shear[12]',color=csh)
