...
shear_it_1=$(( $strain_cycle1 * $Nstep_per_strain));
shear_it_2=$(( $strain_cycle2 * $Nstep_per_strain));
...
echo -e ""- Number of time steps 1 deformation cycle: "${shear_it_1}" >> $file_name;
echo -e ""- Number of time steps 2 deformation cycle: "${shear_it_2}" >> $file_name;
echo -e ""- Number of time steps for Relax steps: "${relaxTime}" >> $file_name;
echo -e ""- Save every "${Ndump}" time steps for dumps files"" >> $file_name;
echo -e ""- Save every "${Nsave}" time steps for fix files"" >> $file_name;
...
echo -e "${shear_it_1}" >> $file_name;
echo -e "${shear_it_2}" >> $file_name;
echo -e "${relaxTime}" >> $file_name;
echo -e "${Ndump}" >> $file_name;
echo -e "${Nsave}" >> $file_name;
...
data_name=""data.longShearPhi"${phi_aux}"NPart"${N_particles}"damp"${damp_aux}"T"${T_aux}"cCL"${CL_con}"ShearRate"${shear_aux}"-Nexp"${Nexp}";
...
echo -e "" >> $file_name;
echo -e "env OMP_RUN_THREADS=1 mpirun -np ${nodes} lmp -sf omp -in in.shear.lmp -var temp $T -var damp $damp -var tstep $tstep_defor -var shear_rate $shear_rate -var shear_it1 $shear_it1 -var shear_it2 $shear_it2 -var Nsave $Nsave -var NsaveStress $NsaveStress -var Ndump $Ndump -var seed3 $seed3  -var relaxTime $relaxTime -var Dir $info_name -var dumpDir $dump_name -var dataName $data_name" >> $file_name;
