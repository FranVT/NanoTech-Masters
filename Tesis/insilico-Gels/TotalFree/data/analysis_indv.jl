...
lblaux_T=map(s->Int64.(round(s[5])),parameters);
lblaux_damp=map(s->Int64.(round(s[6])),parameters);
...
series!(ax_t,time_assembly,reduce(hcat,map(s->s[1],data_assembly))',color=:tab10)
series!(ax_t,time_deform,reduce(hcat,map(s->s[1],data_shear))',color=:tab10)
vlines!(ax_t,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_tcp,time_assembly,reduce(hcat,map(s->s[2],data_assembly))',color=:tab10)
series!(ax_tcp,time_deform,reduce(hcat,map(s->s[2],data_shear))',color=:tab10)
...
series!(ax_E,time_assembly,reduce(hcat,map(s->s[5],data_assembly))',color=:tab10)
series!(ax_E,time_deform,reduce(hcat,map(s->s[5],data_shear))',color=:tab10)
vlines!(ax_E,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_Elo,eachindex(time_assembly),reduce(hcat,map(s->s[5],data_assembly))',color=:tab10)
series!(ax_Elo,eachindex(time_deform).+length(time_assembly),reduce(hcat,map(s->s[5],data_shear))',color=:tab10)
...
series!(ax_Ut,time_assembly,reduce(hcat,map(s->s[3],data_assembly))',color=:tab10)
series!(ax_Ut,time_deform,reduce(hcat,map(s->s[3],data_shear))',color=:tab10)
vlines!(ax_Ut,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_wca,time_assembly,reduce(hcat,map(s->s[6],data_assembly))',color=:tab10)
series!(ax_wca,time_deform,reduce(hcat,map(s->s[6],data_shear))',color=:tab10)
vlines!(ax_wca,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_patch,time_assembly,reduce(hcat,map(s->s[7],data_assembly))',color=:tab10)
series!(ax_patch,time_deform,reduce(hcat,map(s->s[7],data_shear))',color=:tab10)
vlines!(ax_patch,last(time_assembly),linestyle=:dash,color=:black)

series!(ax_swap,time_assembly,reduce(hcat,map(s->s[8],data_assembly))',color=:tab10)
series!(ax_swap,time_deform,reduce(hcat,map(s->s[8],data_shear))',color=:tab10)
...
series!(ax_Ut,time_deform.-last(time_assembly),reduce(hcat,map(s->s[3],data_shear))',color=:tab10)
...
series!(ax_wca,time_deform.-last(time_assembly),reduce(hcat,map(s->s[6],data_shear))',color=:tab10)
...
series!(ax_patch,time_deform.-last(time_assembly),reduce(hcat,map(s->s[7],data_shear))',color=:tab10)
...
series!(ax_swap,time_deform.-last(time_assembly),reduce(hcat,map(s->s[8],data_shear))',color=:tab10)
...
series!(ax_stressXX,time_shear,reduce(hcat,map(s->s[12],data_shear))',color=:tab10)
...
series!(ax_stressXY,time_shear,reduce(hcat,map(s->s[13],data_shear))',color=:tab10)
...
ax_DisplCl = Axis(fig_Displ[1,1:2],
                   title = L"\mathrm{Mean~displacement of Cross Linkers}",
                   xlabel = L"\mathrm{Time [tau]}",
                   ylabel = L"\sigma",
                   titlesize = 24.0f0,
                   xticklabelsize = 18.0f0,
