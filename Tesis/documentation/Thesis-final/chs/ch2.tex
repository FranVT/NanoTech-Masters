...
In conclusion, multidirectional physical \cor{quantities} that are invariant by coordinate changes can be represented through a tensor.
...
\begin{gather}
    \gamma_{ij} = \frac{1}{2}\left( \pdv{u_i}{x_j}+\pdv{u_j}{x_i} \right).\label{eqn:strainTensor}
\end{gather}
...
\begin{figure}[ht!]
    ...
    \caption{Deformation of an infinitesimal rectangular material element~\citep{rahimiEffectPureShear2017}.}\label{fig:strainTensor}
\end{figure}

% \citep{StrainWikipedia}
...
\begin{gather}
    \sigma_{ij} = C_{ijkl}\gamma_{kl}\label{eqn:elasticEQN}
\end{gather}
...
In contrast, a plastic deformation occurs when the material experiences an irreversible change in shape or size and has a \cor{time- independent} response.
...
\begin{gather}
    \dot{\bm{\gamma}}_{p} = f\left(\bm{\sigma},\mathrm{Internal~varaibles}\right),\quad\bm{\sigma}>\bm{\sigma}_{y}\label{eqn:plasticEQN}.
\end{gather}
$\bold{\sigma}_y$ is the yield stress, and $\dot{\bm{\gamma}}_{p}$ is the time derivative of the strain after reaching the plastic deformation.
...
\end{gather}
where $\bm{s}$ is the deviatoric stress tensor\footnote{The stress tensor without the diagonal elements.} and $:$ is a tensor contraction, which maps a tensor to a scalar\footnote{The tensor contraction is defined as \[\bm{\sigma}:\bm{\gamma}=\Tr\left(\bm{\sigma}^T\cdot\bm{\epsilon}\right).\]
        Using the Einstein notation it can be express as \[\bm{\sigma}:\bm{\gamma}=\sigma_{ij}\epsilon_{ij}.\]
    }.
...
Nonetheless, certain materials do not deform when subjected to stress below a specific threshold and exhibit fluidity after this barrier is surpassed, referred to as "yield stress materials" \citep{bonnYieldStressMaterials2017}.
The mechanical reaction depends on shear rate, or deformation rate.
A Newtonian fluid has a linear relationship between these functions: $\sigma=\eta\dot{\gamma}$, where $\eta$ represents constant viscosity.
In yield stress materials, viscosity is a function of shear rate, $\sigma=\eta(\dot{\gamma})\dot{\gamma}$, resulting in a non-linear flow curve.
One common yield stress model is the Herschel-Bulkley model,
\begin{gather}
...
%Although this definitions are not use to compute the yield stress of the simulated polymeric gel, are added to show that this definitions are in terms in the 
...
Lastly, \cor{a viscous response is characterized by a continuous flow under applied stress.}
This response describes materials that continuously deform and undergo an energy dissipation process through viscous friction \cor{and does not present yield stress.}
The constitutive equation for viscous deformation usually follows Newton’s law of viscosity,
\begin{equation}
    \bm{\sigma} = \eta\dot{\bm{\gamma}},\label{eqn:viscousResponse}
\end{equation}
where $\eta$ is the dynamic viscosity and $\eta\dot{\bm{\gamma}}$ is the strain rate. 
...
Which can be thought of as a combination of a viscous response with an \cor{elastic} response.
...
\begin{gather}
    \sigma(t) = \int_0^t G(t-\tau)\dv{\tau}\gamma(\tau)d\tau\label{eqn:viscoelastiEQN}.
\end{gather}
...
\begin{gather}
    \sigma + \lambda\dv{\sigma}{t} = E\gamma+\eta\dv{\epsilon}{t}.
\end{gather}
\end{comment}
...
\begin{gather}
    \dot{\bm{\gamma}}_{p} = f\left(\sigma,\sigma_y,\dot{\gamma},\mathrm{Internal variables}\right)\label{eqn:viscoplastiEQN}.
\end{gather}
...
\begin{gather}
    \sigma = \sigma_y + \eta\dot{\gamma}\mathrm{~for~}\sigma>\sigma_y
\end{gather}
\end{comment}
...
\begin{figure}[ht!]
    ...
    Image retrieved from~\citep{wangThermoviscousHydrodynamicsNonDegenerate2022}.}\label{fig:stressTensor}
\end{figure}

%\citep{StressWikipedia}
...
Equation~\eqref{eqn:DerVirTen5} is referred to as the \textit{dynamical tensor virial theorem} and serves as an alternate representation of linear momentum equilibrium.
This theorem is applicable upon assuming the existence of a time scale $\tau$, which is small in comparison to macroscopic processes yet substantial relative to the characteristic time of the particles in the system, during which the particles remain near their initial positions with constrained positions and velocities.
Utilizing this characteristic, we can calculate the temporal average of~\eqref{eqn:DerVirTen5}.
\begin{equation}
...
\end{equation}
Assuming that $\vec{r}_\alpha\otimes\vec{p}_\alpha$ is bounded, and that the temporal scales between microscopic and continuum processes are sufficiently large, the term on the left-hand side can be minimized by selecting a sufficiently large $\tau$.
By summing over all particles, the \textit{tensor virial theorem} is:
\begin{equation}
...
\end{equation}
Here, $\vec{\xi}$ denotes a position vector located within the domain $\Omega$ that is occupied by the system of particles, which is bounded by a continuous closed surface $\delta\Omega$.
 Assuming that $\Omega$ is sufficiently large to represent the external forces operating onto it as the continuum traction vector $\vec{t}$.
...
However, plastic deformation is is often described as \cor{the irreversible rupture of crosslinks or polymer chains within the network} or stretching of individual chains beyond their reversible limit.
\cor{Compromising the integrity of the polymeric network.}
...
This energy dissipation mechanism facilitates reversible recovery from mechanical impacts, enabling the development of rigid and resilient hydrogels.
Energy dissipation is frequently accomplished by incorporating sacrificial connections by the addition of a secondary, more brittle network within a loosely cross-linked framework or by utilizing cross-links created from supramolecular transient interactions, which can be reversibly disrupted and reconstituted.
In the initial scenario, the material undergoes stretching, resulting in the rupture of some links within the more fragile network, which therefore enhances the dissipation of energy from mechanical impact.
In the second scenario, the sacrificial bonds are non-covalent and can reversibly dissociate and reconstitute.
...
Non-covalent interactions include metal-ligand interactions, ionic contacts, hydrogen bonding, microphase separation, hydrophobic interactions, polymer-nanomaterial adsorption, host-guest complexation, and supramolecular self-assembly. 
The electrostatic process can produce strong, physically resilient polymers by ionic interactions between oppositely charged monomers.
Moreover, hydrogen bonding can serve as a potent motif to augment reversible interactions among polymer chains, thereby dissipating energy.
...
The composite mechanism posits that the incorporation of micellar aggregates and soft polymer microspheres enhances the toughness of the hydrogel.
Furthermore, (nano)fibers can significantly improve the fracture mechanics of robust hydrogels; nonetheless, their ductility frequently diminishes, particularly with the incorporation of macroscopic fiber mats.
Ultimately, the self-assembly mechanism indicates that elastomeric proteins, which operate as molecular springs, present a possible source of energy dissipation by unfolding under applied stress. 
This mechanical unfolding occurs in both larger proteins and smaller elastin-like polypeptides.
...
\subsection{\cor{Stress relaxation of a dual network}}
...
\subsection{\cor{Strain stiffening in dual networks}}

Reference~\citep{kongEffectCrossLinkHomogeneity2024} details the fabrication of randomly cross-linked networks of poly(n-butyl acrylate) through thermally induced free-radical polymerization, as well as regularly cross-linked networks via thiol-bromine linking of tetrafunctional poly(n-butyl acrylate) star polymers.
 They subsequently increase both types of networks inside a combination of monomers and cross-linkers, which are then polymerized to create double networks. 
 This study seeks to investigate the impact of cross-link uniformity on the strain stiffening characteristics of both regular and random networks.
...
Randomly cross-linked networks typically exhibit a polydisperse distribution of strand lengths, characterized by a significant proportion of short strands, leading to strain hardening at comparatively low extensions. 
In contrast, regular networks demonstrate a significantly smaller distribution that includes primarily higher molecular weight strands and are expected to exhibit significantly delayed strain stiffening compared to analogous random networks.
...
\begin{figure}[ht!]
    ...
    \caption{Stress-strain curves for single networks obtained from \cor{randomly (panel (1a) of figure~\ref{fig:RANREGnetworks}) cross-linked networks}.
...
\begin{figure}[ht!]
    ...
    \caption{Tensile response of single networks \cor{obtained from regular (panel (1b) of figure~\ref{fig:RANREGnetworks}) cross-linked networks with matched target molecular weight between crosslinks}.
...
\begin{figure}[ht!]
    ...
    \caption{Tensile response of \cor{double network with random cross-link network (panel (a)) and regular cross-linked network (panel (b)).
            Panel (a) is the mechanical response of panel (2a) in figure~\ref{fig:RANREGnetworks}.
            Panel (b) is the mechanical response of panel (2b) in figure~\ref{fig:RANREGnetworks}.
    }\citep{kongEffectCrossLinkHomogeneity2024}.}\label{fig:REGRANDNnetworks}
\end{figure}

\begin{figure}[ht!]
    \centering
    \includegraphics[width=12cm]{figs/explainMechResponse/comparissonREGRANDN.jpeg}
    \caption{\cor{Comparisson of the mechanical response between single and double network cross-linked networks.
        In panel (a) are shown the single network configurations, black line is the response of panel (1a) in figure~\ref{fig:RANREGnetworks}, and the red line is the response of panel (1b) in figure~\ref{fig:RANREGnetworks}.
        In panel (b) are shown the double network configurations, where the black line is the response of panel (2a) in figure~\ref{fig:RANREGnetworks}, and the red line is the response of panel (2b) in figure~\ref{fig:RANREGnetworks}
        }~\citep{kongEffectCrossLinkHomogeneity2024}.}\label{fig:REGRANDNcomparison}
\end{figure}


\subsection{\cor{About shear thinning}}

In reference~\citep{varela-feijooMultiscaleInvestigationViscoelastic2023} \cor{it is reported} that an alginate hydrogel presents shear-thinning, as shown in figure~\ref{fig:hydroMechResponse2}.
...
\begin{figure}[ht!]
    ...
    \caption{\cor{Variation of stationary shear viscosity with shear rate in aqueous sodium alginate solutions at various concentrations throughout the semi-dilute and concentrated regimes. Solid lines represent the fitting of experimental data according to the Cross model~\citep{varela-feijooMultiscaleInvestigationViscoelastic2023}.}}\label{fig:hydroMechResponse2}
\end{figure}

\subsection{\cor{Relevance of crosslinkers}}
...
In both experiments, it is reported that for the initial deformation, up to 15\% of relative deformation, the relationship is linear, after which it transitions to an exponential relationship.
This response is attributed to the charged side chains of the hydrogel.
As the tension in the polymer increases, the proximity of the similarly charged functional groups intensifies.
This aversion results in heightened stress as strain increases.
Nonetheless, in the identical experiment utilizing an alternative monomer, they document a divergent reaction, illustrated in figure~\ref{fig:mechResponseEnd1}.
Following a significant increase in stress at minimal stresses, a precipitous decline was succeeded by a more gradual reduction in stress.
This reduction may be attributed to the hydrogel's desiccation during the measurement process.
...
It was also \cor{mentioned} how energy is dissipated by sacrificial connections and reversible non-covalent cross-links, which improve hydrogel toughness through microcrystallization and composite mechanisms. 
...
In order to understand how molecular dynamics helps to simulate the mechanical response of hydrogels, it is need to \cor{introduce} the concept of colloids.
...
\cor{It is important to acknowledge that here, gel it is refer to as a \emph{polymeric gel} not a \emph{collidal gel}.}
\cor{Colloidal gels are structures made up of a network of colloidal particles\citep{jiangColloidalGelationNonsticky2023,royallProbingColloidalGels2015}, meanwhile, polymeric gels are polymer networks\citep{formanekGelFormationReversibly2021b,safferSANSStudyHighly2014}.}
...
The Langevin theory utilizes the densities of the liquid and solid parts, considering the colloidal representation of the hydrogel.
Due to the substantial mass of the solid phase in the colloid, its momentum will alter after numerous collisions with water molecules, resulting in a system of heavy particles that operates on a far longer time scale than that of the solvent molecules~\citep{Thijssen2007}.
The disparity in time scales can be advantageous for omitting the details of the solvent particles' degrees of freedom, thereby representing their influence through stochastic and dissipative forces, which facilitates longer simulations that would be unfeasible with explicit solvent inclusion~\citep{pastorTechniquesApplicationsLangevin1994}.
...
\cor{In simulation literature, it is typically called collision frequency, despite the fact that a rigorous Langevin description suggests that the solute experiences an unlimited number of collisions with infinitesimal momentum transfer.{pastoral techniques applications Langevin 1994}.
Moreover, the absence of dependence of the second term on the position of any particles indicates the neglect of hydrodynamic interactions or spatial correlations in the friction kernel~\citep{pastorTechniquesApplicationsLangevin1994}.}
...
\end{align}
The noise is depicted as a singular stochastic variable for each time step, manifested as a single aggregated impulse during $dt$ that affects the dynamics throughout the time step.
Ultimately, it is crucial to recognize that $\vec{F}^{n}$, $\vec{F}^{n+1}$, and $\vec{\cor{\rho}}^{n+1}$ possess units of momentum \SI{}{\kilo\gram\meter\per\second}, while the constant $\frac{m dt}{2m +\cor{\mu} dt}$ has units of time \SI{}{\second}.
...
To establish a simulation using this software, it is necessary to generate an input script.
This input script consists of multiple lines, each commencing with a command name and succeeded by one or more arguments delineated by whitespace.
The program includes programming commands that establish variables, conduct conditional tests, execute loops, or call shell commands to initiate an external program.
The input script is analyzed and run sequentially, line by line. 
This capability enables a singular script to execute a simulation in phases, modify one or more parameters between phases, or conduct a series of independent simulations wherein the complete system is reinitialized many times~\cite{LAMMPS}.
...
The \cor{second term} is equivalent to $-m\cor{\mu} v$ and $F_r$ is equivalent to the term $\vec{R}(t)$ on equation~\eqref{eqn:BrownianDyn1}.
...
The magnitude of the stochastic force is derived from the fluctuation-dissipation theorem, where $k_B$ denotes the Boltzmann constant, $T$ represents the target temperature, $m$ indicates the particle's mass, $dt$ symbolizes the timestep size, and $\mathrm{damp}$ refers to the damping coefficient.
The damp factor is inversely related to the viscosity of the solvent.
A short relaxation period signifies a high-viscosity solvent, and vice versa.
This parameter is expressed in temporal units and determines the rate of temperature equilibration~\citep{LAMMPS}.
