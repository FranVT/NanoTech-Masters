...
the temperature was set to be constant through all the assembly process, $\cor{T^*}=\num{0.05}$
%\paragraph{Langeving parameters}

\cor{The $\mathrm{damp}$ parameter was fixed to a value of \num{0.1}~$[1/t^*]$ from debugging during the shear deformation simulation.
At the begginig the parameter was set to \num{1} or slightly bigger values.
However those values causes big fluctiations in the temperature and the relaxed temperature values was higher than the desired value.
In order to find why that happen, the damp parameter was strongly reduce to \num{0.1} and it was found that the implementation of the threebody potential was wrongly implemented.
This is going to be exaplained in detail in subsequent sections\ref{subsec:LammpsImplementation}.
Once this implementation was fixed, the damp parameter wasn't changed.
}
...
\cor{Spatial and time average.}\com{What type of average?}
...
The time average for the assembly protocol is specified in terms of the damp parameter: $100\cdot\cor{\mathrm{dt}^*}\cdot\cor{\mathrm{damp}^*}$, whereas for the shear deformation, the period was set \cor{such that in the time period the strain achieves} $0.05\cor{\gamma^{*}}$.
