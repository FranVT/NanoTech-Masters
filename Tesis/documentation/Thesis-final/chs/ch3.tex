...
A flexible numerical portocol capable of designing these individual microgel particles has been presented in the \cor{literature}~\citep{gnanSilicoSynthesisMicrogel2017}.
The authors introduce a flexible numerical method that can imitate \emph{individual microgel particles} made from PNIPAM crosslinked networks.
\cor{In this work, the numerical method is modified to create a polymeric gel rather than a microgel particle.
To achieve that, the spherical confinement and the temperature-sensitive swelling potential are removed from the methodology.}

As a result, the central technique involves using a binary mixture of patchy particles to create a disordered polymeric network structure, \cor{afterwards subjected to shear deformation.} 
...
%The authors present a flexible numerical protocol capable of designing \emph{individual microgel particles} based on PNIPAM crosslinked networks\com{Why would be different for other hydrogles?}. 
...
This potential it is used to represent covalent bonds between monomers in a polymers\citep{leRheologyStructuralProperties2008}.
...
Figure~\ref{fig:patchpatchpot} compares the patch-patch interaction potential, the Lennard-Jones potential, and the FENE potential \cor{with $K=1$, $R_o=1$, $\epsilon =1$, $O=1$ in order to explore a qualitative comparisson.}\com{Poner eso en la caption}
...
However, if the simulation for the polymeric network is simply performed using the \cor{WCA} and patch-patch potentials, the patches will form interactions of more than two patches, which is undesirable.
...
\begin{align}
    E_r &= K_r\qty(r-\cor{r_{bo}})^2, \\
    E_\theta &= K_\theta\qty(\theta-\cor{\theta_{bo}})^2.
\end{align}
...
\cor{For the patchy particles, the values of $\cor{K_r^{*}}$ and $\cor{K_\theta^{*}}$ are established at \num{100} to create a robust bond and inhibit the elongation of monomers (patchy particles B) and crosslinkers (patchy particles A).
The distance between the center of the central particle and the patches is set to \num{0.45}$r^*$.
The value of $\theta_o$ is established at \SI{180}{\degree} for PB particles and \SI{109.4712}{\degree} for PA particles.}

\cor{Because of constraints concerning time and computational resources, the total patchy particle number is established at $N_p=\num{8000}$\com{Size effect.}.
This represents a reduced quantity of particles in comparison to other simulations~\citep{gnanSilicoSynthesisMicrogel2017}.
Consequently, to offset this, we compute the average of five simulations of the shear deformation.}
\cor{Furthermore, the time step is established at \num{0.001}$t^{*}$ to guarantee numerical stability. 
The stability criterion is expressed as $\mathrm{damp}^{*}dt^{*}<2$~\citep{leimkuhlerContractionConvergenceRates2024}.
Given that $\mathrm{damp}^{*}dt^{*} = \num{1d-4}$, numerical stability is guaranteed.
This enables us to obtain acceptable average observables.}
...
\begin{figure}[ht!]
    ...
    \caption{Panels (a) and (c) show the system with patchy particles.
        ...
        After and before shear\com{Cuanto shear rate?}.
...
At higher \cor{concentrations}, the stress rises until it reaches its maximum level.
