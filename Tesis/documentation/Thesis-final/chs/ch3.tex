...
Meanwhile, \cor{$K_{br}$} and \cor{$K_{b\theta}$} are equal to $k/2$, where $k$ is the energy of the bond.
\cor{This allows placing the patches at desired positions.
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=12cm]{figs/numerical/patchpatch.pdf}
    ...
    The blue and pink lines represent the Lennard-Jones and FENE potentials, which are the commonly used to represent interparticle interactions.}
    \cor{For comparison purposes, the parameters are set to \num{1}, $K=1$, $R_o=1.5$, $\epsilon=1$, $D=1$.}
}\label{fig:patchpatchpot}
\end{figure}

\cor{In contrast with the well-known Lennard-Jones potential, the minimum of the patch-patch potential is at the distance equal to the diameter of the particle, meanwhile the Lennard-Jones potential is $0$ at that distance.
...
This is more appropriately depicted in Figure~\ref{fig:patchpatchpot}.
}
...
This potential is defined in terms of the pair potential of eq.~\eqref{eqn:patch-patch_interaction}, which facilitates an efficient bond-swapping mechanism, enabling the system to equilibrate readily even at low temperatures, while simultaneously maintaining the single bond per patch condition~\citep{sciortinoThreebodyPotentialSimulating2017}.
...
The sum in~\eqref{eqn:swap_interaction} runs over all triplets of bonded patches (patch $l$ bonded both with $m$ and $n$).
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=12cm]{figs/numerical/swapPotential.pdf}
    \caption{Swap potential for patch-patch interaction to ensure single bond per patch condition.
        Dark blue line represents the patch-patch interaction potential.
        Orange, green, pink and light blue lines represent the swap potential with a fix distance between patch $i$ with patch $j$ and a free patch $k$.
        When the patches $i$ and $j$ are at the potential well, the interaction between patches $j-k$ is repulsive.
...
The patchy particles featuring an orange central particle represent monomers due to the presence of two patches.
The patchy particle featuring a red central component indicates the crosslinker due to its four patches, indicating a valence of 4.
...
\cor{Before detailing the numerical values for the parameters, it is crucial to explain that the numerical simulations are performed using reduced Lennard-Jones units for computational and physical efficiency.
...
\end{equation}
\cor{Observing that $k_{\mathrm{B}}$ serves as a conversion factor between temperature and energy, this decision facilitates a straightforward comparison of thermal energy with interaction energy.
...
\cor{For both patchy particles, the values of $\cor{K_{br}^{*}}$ and $\cor{K_{b\theta}^{*}}$ are established at \num{100}~\cor{$\epsilon^*/(r^*)^2$} to create a robust bond and inhibit the elongation of monomers and crosslinkers.
...
Finally, the value of $\theta_{bo}$ is established at \SI{180}{\degree} for PB particles and \SI{109.4712}{\degree} for PA particles and the amount of patches and angles corresponds to that described in~\citep{gnanSilicoSynthesisMicrogel2017}.}
\cor{The inter-center distance was adjusted to ensure that the centers do not coincide with the surface of the central particle to avoid computational artifacts and to avoid zero range molecular interactions.}
...
\cor{The temperature is established at $\cor{T^*}=\num{0.05}$.
 ...
 Consequently, once the network is formed, it is exceedingly unlikely for it to reorganize spontaneously because the energy is insufficient to disrupt the patch-patch interactions.
...
The width of the time window for the assembly process is defined as $\num{100}\cdot dt\cdot\mathrm{damp}$.
Meanwhile the width of the time window for the shear is defined as $\num{0.05}/(\dot{\gamma}^{*} dt)$.}
...
Following finishing the network assembly, the final configuration was stored in a file before the application of shear deformation.
 The patchy particle network was subsequently exposed to a sequence of five deformations with corresponding shear rates.
...
The reported measurements are rolling mean temporal averages.
The width of the time window for the rolling mean of the assembly and shear protocols regarding other observables, including energy and temperature, are identical to those established for the stress observables.
...
\cor{The patchy particles that are not within the primary cluster, were part of minor clusters, or did not form bonds were left untouched in the simulation box.
...
Then it continous to deform until achieving a strain of \num{1}, as shown in panel (d).
...
\begin{figure}[ht!]
    ...
    \caption{A graphical representation of the shear deformation applied to the patchy particle network in the $xy$ plane.
            ...
            Finally, panel (d) shows that the box enters a configuration with no tilt but the system has a strain of \cor{$(\gamma^{*}=1)$}.
...
\cor{Additionally, Figure~\ref{fig:shearBonds} shows the system before (panels (a) and (b)) and after the (panels (c) and (d)) shear deformation.}  
...
This behavior has also been documented in the mechanical response of \cor{a simulation of a polymeric gel created using a custom variation of the random Gaussian network generation algorithm\citep{argunInterplaySpatialTopological2024}} and in other polymeric systems\citep{osakiStressOvershootPolymer2000a,ravindranathUniversalScalingCharacteristics2008,boukanyUniversalScalingBehavior2009}.

\cor{The sudden increase in the stress value is commonly referred to as \emph{stress overshoot}, and} has been thoroughly researched, as outlined in relevant literature, including molecular dynamics simulations \citep{jeongEffectChainOrientation2017,caoSimulatingStartupShear2015,mohagheghiMolecularlyBasedCriteria2016,baigFlowEffectsMelt2010a} and an analysis in other articles by \citep{wangExploringStressOvershoot2009}.
...
\begin{figure}[ht!]
    ...
    Each line represent a 5 experiments average with an assembly average and a moving time average.
    \cor{The overshoot phenomena can be potentially explained by the alignment of the patch-patch bond with the direction of the deformation.}}\label{fig:stres-strainResults}
...
Subsequently, figure~\ref{fig:stress-strainCLResults} shows the strain-stress relationship of three systems with different crosslinker concentrations at shear rates of \num{1d-3}, \num{5d-3}, and \num{10d-3}.
...
This interpretation allows us to reconsider the breaking of pairwise interactions as a relevant phenomenon in the shear process.
}

\cor{With a temperature value of $T^{*}=\num{0.05}$ and a damping factor of $\num{0.1}[1/t^*]$, the mechanical response is expected to be brittle and elastic rather than exhibiting viscous flow.
...
The peak strain value and peak height rise proportionally with the shear rate.
Panel (c) of figure~\ref{fig:stress-strainCLResults} illustrates the stress-relaxation behavior of viscoelastic materials.
Moreover, the crosslinker is seen to enhance the viscoelastic response by elevating the maximal stress before stress relaxation occurs.
It is noteworthy that in figure~\ref{fig:stress-strainCLResults}, the maximum stress levels are observed prior to a strain value of $2$, which contrasts with previous studies \citep{jeongEffectChainOrientation2017,janeschitz-krieglPolymerMeltRheology1983,pearsonFlowInducedBirefringenceConcentrated1989,masubuchiPrimitiveChainNetwork2020} that indicated peak stress occurs at strain values approximately between $2-3$.
Moreover, the strain shift at peak load is imperceptible, \cor{therefore indicating a difference.}
\cor{This implies that our network lacks the elasticity observed in the literature, which can be attributed to the value of the damping parameter.}

Another intriguing finding is the change in the steady-stress regime after the overshoot at different shear rates.
...
\cor{Analyzing identical crosslinker concentrations at different shear rates reveals a rise in stress, indicating a decrese in viscosity.
This phenomenon can also be referred to as \emph{shear thinning}.
Thus, this corroborates the interpretation of network fragmentation, as systems exhibiting shear thinning are explained by network fragmentation.
And this response is consistent throughout all the tested concentrations.}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.9\textwidth]{figs/ComputaitonalResults/yieldStress.pdf}
    \caption{Time average of steady-stress regime from computational results. 
        Dashed lines represent the Herschel-Bulkey model, indicating a shear thinning of a viscoelastic fluid.
    }\label{fig:yieldStressResults}
\end{figure}
\cor{Finally, in order to test the viscoelastic response of the system, the steady stress values at big strains is fit with the Herschel-Bulkey model for yield stress mateirals.}
The fit is as follows,
\begin{equation}
    \sigma^{*}_{xy}(\dot{\gamma}^{*}) = c_1 (\dot{\gamma}^{*})^{c_2}+c_3.\label{eqn:fit}
\end{equation}
\cor{The restrictions for the parameters $c_2<1$ and $c_3>0$ are established to ensure physical interpretation as suggested in~\citep{bonnYieldStressMaterials2017}.
Table~\ref{tab:fitParameters} shows the numeric values of the fit~\eqref{eqn:fit} for each system with different crosslinker concentration.}
\begin{table}[ht!]
\centering
\caption{\cor{Parameters of the fit model}}\label{tab:fitParameters}
...
\cor{The values for $c_2$ support the interpretation of the system as a network exhibiting shear thinning.
Conversely, the values of $c_3$ approaching \num{0} suggest the absence of a yield stress.
Ultimately, $c_1$ is understood as the resistance to flow, signifying that the network facilitates flow easier with a larger crosslink concentration.
}
\cor{The close proximity of the numeric values of the yield point fit to zero suggests that the polymeric network is better characterized as a viscoelastic fluid rather than a viscoelastic solid.
This could be explained by the patch-patch pairwise interaction.
Due to the ability of the path-patch interaction to break and reassemble, it exhibits minimal resistance of the network to shear deformation.
Inhibiting the yield stress phenomena.}
\cor{Ultimately, it is crucial to recognize that the low temperature significantly affects the mechanical response, which is largely governed by the patchy particle  \emph{backbone} network.
Elevating the temperature may alter the mechanical response, thereby affecting the parameters of the fitting model.}
