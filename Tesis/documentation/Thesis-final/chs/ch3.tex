...
Finally, by setting $k_B$ to $1$ with the other quantities, the time is rescaled by $t^* = t\sqrt{\epsilon/(m_{\mathrm{ref}}D^2)}$.}
...
\cor{In order to reduce the contribution of energy dissipation in the mechanical response, the $\mathrm{damp}$ parameter is set to \num{0.1}~$[1/t^*]$.
...
The diameter of the central particle is set to $\cor{O^*}=1$ and the diameter of the patches at $\cor{O_p^*}=0.4$.
...
%\paragraph{damp}
Comparing equation~\eqref{eqn:MolDylammps1} with equation~\eqref{eqn:MolDylammps2}, the viscosity parameter $\gamma$ of the Langevin equation~\eqref{eqn:BrownianDyn1} does not appear; instead, the $\mathrm{damp}$ parameter appears.
