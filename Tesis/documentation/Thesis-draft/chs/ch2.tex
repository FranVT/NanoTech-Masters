...
\end{equation}
Now we need are going to tackle the integral of force in equations~\eqref{eqn:DerVelVerlet3,eqn:DerVelVerlet4}.
Since is a deterministic force, we can approximate the integral such that both equations are correct to second order in $dt$.
\begin{align}
    \vec{r}^{n+1} &= \frac{dt}{2+\gamma dt}\left[ 
                        2\vec{v}^{n}                
                        +\frac{1}{m}\int_{t_n}^{t_{n+1}}\vec{F}(t)dt'
                    ...
                    \right]
                    +r^n, \\
    \vec{v}^{n+1} &= \frac{1}{m}\frac{dt}{2}\qty(\vec{F}^{n} + \vec{F}^{n+1})
                     -\gamma\qty(r^{n+1}-r^n)
                     +\frac{1}{m}\int_{t_n}^{t_{n+1}}\vec{R}(t')dt'
                     +\vec{v}^{n}.
\end{align}
