...
From a mathematical point of view, this algorithm is a second-order integrator for ordinary differential equations.
And is based on truncated Taylor expansion, which is defined as
\begin{align}
...
The Stormr-Verle method is when we consider two successive time steps and eleminating the velocity variables.
which gives,
\begin{align}
...
It is important to notice that the difficulty to develop accurate integrators for Langevin Dynamics stems from the non-analytic nature of $\beta(t)$, which invalidates the Taylor expansion commonly used for the derivation of the Verlet scheme.
...
Now we are going to derive the Stormer-velocity verlet scheme for the Langevin equation following the process shown in \citep{gronbech-jensenSimpleEffectiveVerlettype2013a}.
First we integrate eqation[eqn] over a small time interval $dt$,

With no approximation, this can be written as,

cosndiering that we have a Gaussian random number with expected value and variance.

Now, y integrating over we get,

approximating the integral using finite differences and by substituting eqn[eqn] into ...
