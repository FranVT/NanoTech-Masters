...
A hydrogel is a polymeric network capable of swelling~\citep{choiSynthesisHydrogelBasedMicrogels2025,dayTunableNetworkArchitecture2025,ahmedHydrogelPreparationCharacterization2015a,bustamante-torresHydrogelsClassificationAccording2021}.
...
In an effort to comprehend (some of) the features of hydrogels, we will first define a polymeric network and its swelling capacity.
...
From a structural perspective, polymer networks consist of network \emph{junctions}, which contain three or more strands connected by some mechanism.
...
Some of the key hydrophilic groups are, hydroxyl group, amide group, carboxylate anion, or ether group.
...
The underlying principles of the crosslink mechanism are either the physical interactions or covalent bonds. 
...
Alternatively, the incorporation of mechanisms for covalent bond exchange can result in chemical networks that exhibit adaptable mechanical properties regulated by external stimuli~\citep{hanDynamicCovalentHydrogels2022,mariopereraDynamicCovalentBonds2020,chenCovalentAdaptablePolymer2024}. 
...
In contrast, physical interactions are attributed to electrostatic, van der Waals, or dipole forces, arising from the redistribution of electron density and the consequent energy alterations between particles [\textbf{cite}].
There also can be identified a third type of crosslinker mechanism, which is when the polymer chains are physically entangled, and this phenomenon is commonly known as mechanical crosslinking.

The next section discusses several polymer network types and cross-linking processes.
The essential ideas of mechanical reactions are then presented using constitutive relationships. 
Finally, the section will present an overview of hydrogel mechanical processes, including the molecular events that underpin some of them.

\subsection{Types of polymeric networks: Gels}

%\paragraph{Introduction}
In general, a polymeric network can be classified into four major categories: thermosets, thermoplastics, elastomers, and gels [\textbf{cite}].
...
In contrast, thermoplastics are held together by strong physical interactions, allowing them to be remolded and recycled when heated\footnote{Add examples in the paragraph}. 
...
Also referred to as the gel point defined in the context of percolation theory. 
...
%\paragraph{Intro} 
Now that the importance of the crosslinking mechanism for creating a gel material has been introduced, let's explore in more detail this phenomenon. 
According to the literature, crosslinking processes are most commonly classified as physical or covalent crosslinkers. 
However, it is crucial to emphasize that, while the crosslink mechanism is important for network integrity, it is not the only mechanism capable of accurately describing the mechanical reaction in terms of network topology.  
...
\subsection{Reversible networks}\footnote{Add References}
...
This equation is a mathematical tool that quantifies the relationship. 
It is used to predict or model material behavior, as reflected by empirical observations.
Additionally, these equations can be defined from both the microscopic and macroscopic scale.
These two scales are linked through the implementation of averaging or homogenization procedures.
However, there is a difference in interpretation and application.
...
There are four main strain-stress relations: elastic, plastic, viscoelastic, and viscoplastic\footnote{What about pure viscosity}.
...
An elastic deformation occurs when the material returns to its original configuration once the stress is removed.
...
This relationship also enables the description and modeling of memory effects and rate dependence, which are commonly observed in polymers, biological tissues, and certain soft solids [\textbf{cite}].
...
In comparing the results of Langevin dynamics with those of other stochastic methods, the relevant variable is the velocity relaxation time, $\tau_{v}$, which equals $m\gamma^{-1}$\citep{pastorTechniquesApplicationsLangevin1994}.
...
Finally, it is important to see that $\vec{F}^{n}$, $\vec{F}^{n+1}$, and $\vec{\eta}^{n+1}$ have units of momentum \SI{}{\kilo\gram\meter\per\second} and the constant $\frac{m dt}{2m +\gamma dt}$ has units of time \SI{}{\second}.
