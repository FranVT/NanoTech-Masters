...
\paragraph{Molecular stress is equivalent to continuum stress} Since this is a crucial joint that validate the use of molecular dynamics to analyze the mehcanical response via numerical solutions, the mathematical proof of how the macro-stress tensor is equivalent to the numerical micro-stress tensor is given.
It is important to say that the following derivation can be found in the apendix of\citep{admalUnifiedInterpretationStress2010}\footnote{Describe more if what is done in this article}.\footnote{(Eventualmente pondré esto en párrafo) Notation:
...
This help us to quantify the relation between the network properties with the macroscopic cauchy stress tensor, and therefore start exploring hypothesis of what type of molecular process dominates in each deformation regime.

\paragraph{Elastic regime} Is it explain thru bond stretching

\paragraph{Plastic regime} Commonly explain as molecular dislocation of the lattice. Not sure in other materials

\paragraph{Viscoelastic regime} Bond stretching and rearrangements of grains and stuff.

\paragraph{Viscoelastic regime} Because yes
