...
\paragraph{Elastic relation} An elastic deformation is when the meterial returns to its original configuration once the stress is removed.
The quantification of this response is given by a linear equation\footnote{\begin{gather}
    \bm{\sigma} = E\bm{\epsilon}\label{eqn:elasticEQN}
\end{gather}
},
with a proportionality constant known as elastic modulus or Young's modulus.
The equation~\eqref{eqn:elasticEQN} is interpreted as \textit{how the material resists deformation}.
Some examples of this type of deformation are the streching of a metal spring within its elastic limit, the compression of a rubber ball or the bending of a steel beam under moderate load.
