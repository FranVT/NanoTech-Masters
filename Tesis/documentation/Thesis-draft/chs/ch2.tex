...
In ``dynamic'' covalent bonds like imine, boronate ester, thioester, disulfide, and transesterification, the network can break and rebuild in reaction to high-energy external stimuli.  External cues promote the exchange of polymer chains at crosslinked sites via associative or dissociative mechanisms.
The associative process happens when new bonds develop before existing bonds break.  In contrast, the dissociation mechanism occurs when existing relationships disintegrate before new ones form.
The exchange of polymer chains allows the material to display self-healing, stress relaxation, and form adaptation while retaining the mechanical strength supplied by covalent links in the network.
Finally, it is vital to highlight that the energy barrier for exchange, bond stability, and stimulus dependence all influence the rate and reversibility of the process.
