...
Now, after having shown how molecular dynamics can simulate the mechanical response of hydrogels, allowing the construction of relationships between responses to molecular phenomena, it's time to present the methods for simulating shear deformation in a hydrogel employing patchy particles.
...
In the article \textit{In silico Synthesis of Microgel Particles}\citep{gnanSilicoSynthesisMicrogel2017}, the authors present a flexible numerical protocol capable of designing individual microgel particles based on PNIPAM cross-linked networks. 
...
Figure~\ref{fig:patchpatchpot} compares the patch-patch interaction potential, the Lennard-Jones potential, and the FENE potential\footnote{Add the FENE definition and clarify why we are comparing with this potential.}.
...
\begin{figure}[ht!]
    ...
    \caption{Comparisson between equation~\eqref{eqn:patch-patch_interaction} with Lennard-Jones and FENE potentials\footnote{Add the WCA potential}.}\label{fig:patchpatchpot}
\end{figure}
...
Equation~\eqref{eqn:patch-patch_interaction} represents a more ``strong'' interaction between the patches compared to a Lennard-Jones potential.
...
Hence, the interaction between patches is complemented by a three-body repulsive potential, defined in terms of~\eqref{eqn:patch-patch_interaction}, that provides an efficient bond-swapping mechanism, making it possible to easily equilibrate the system at extremely low temperatures, while at the same time retaining the single bond per patch condition\citep{sciortinoThreebodyPotentialSimulating2017}.
\begin{gather}
...
Taking into account this, when the patches $i-j$ are at the potential well ($r_{ij}=\sigma$), the interaction between $i-k$ is null.
When the patches $i-j$ are bigger than the potential well but smaller than the cutoff distance ($r_{\mathrm{cut}}>r_{ij}>\sigma$), the interaction between $i-k$ is mildly attractive.
Finally, when the patches $i-j$ are bigger than the cutoff distance ($r_{\mathrm{cut}}<r_{ij}$), the interaction between $i-k$ is attractive.

\begin{figure}[ht!]
    \centering
    \includegraphics[width=12cm]{figs/numerical/swapPotential.png}
    \caption{Swap potential for patch-patch interaction to ensure single bond per patch condition\footnote{Be carefull with the sign interpretation and more detail about the different curves.}.}\label{fig:swappot}
\end{figure}
...
This is a lower number of particles when compared with other simulations [\textbf{cites}].
Therefore, in order to compensate, we take the mean of five experiments.
In addition, the timestep was set to \num{0.001}, as a rule of thumb\footnote{Talk about LJ units.}.
This is equivalent to simulating a system of \num{40000} particles, but with more manageable computational requirements.
...
The interaction of patchy particles with two patches is set to 1 ($\epsilon_{\mu,\upsilon}=1$), whereas the interaction with four patches is set to \num{0} ($\epsilon_{\mu,\upsilon}=0$), and the interaction between patches of patchy particles with four patches with patches of patchy particles of two patches is set to \num{1} ($\epsilon_{\mu,\upsilon}=1$).
...
This is because we are exploring this computational methodology to characterize the mechanical response and to deform beyond the plastic deformation limit.
...
%\paragraph{damp}
By making a comparison between equations~\eqref{eqn:MolDylammps1} and~\eqref{eqn:MolDylammps2}, the viscosity parameter $\gamma$ of the Langevin equation~\eqref{eqn:BrownianDyn1} does not appear; instead, the $\mathrm{damp}$ parameter appears.
...
The \verb|pair_style threebody/table| command is used to implement generic tabulated three-body interactions.
...
\end{equation}
And due to symmetry reasons, $f_{i1}=-f_{j1}$, $f_{i2}=-f_{k1}$, and $f_{j2}=-f_{k2}$.
...
Hence\footnote{If time allows, add a figure}, 
\begin{align}
...
With these projections the forces can be expressed in~\eqref{eqn:3body1},~\eqref{eqn:3body1}, and~\eqref{eqn:3body1} as follows:
\begin{align}
...
In addition to the \verb|erate| style, the \verb|remap| keyword was set to \verb|x| to remap\footnote{Explain what is remaping} particle positions without affecting their velocities\citep{LAMMPS}.
...
\paragraph{Explain transition protocol} I haven't describe the assembly to shear protocol.
...
The number of time steps for this stage has been established as a rule of thumb.
...
%Once the assembly protocol was completed, the final configuration was archived in a file before applying a shear deformation.
%The patchy particle network was then subjected to a set of five shear rate-matched deformations.
%Once the metrics from the five simulations had been stored, an average of those experiments was performed.
...
Finally, this process is repeated until the deformation reaches a strain of $15$.
...
On the other networks, the stress rises until it reaches its maximum level.
...
However, it should be noted that in figure~\ref{fig:stress-strainCLResults}, the greatest stress levels occur before the strain value of $2$, which differs from the earlier reports [\textbf{cites}], which reported that the highest stress occurs at strain values of about $2-3$.
...
Another intriguing finding is the change in the steady-stress regime much after the overshoot.
...
The dashed lines represent exponential fits $m(p_1,p_2,\dot{\gamma}) = p_1\dot{\gamma}^{p_2}$, with exponents $\{0.5901,0.4880,0.4045\}$ for the \num{10}\%, \num{5}\%, and \num{3}\% crosslinker concentrations, respectively.
