...
Recalling that the force is equivalent to $-\nabla U(r)$ and the potential has only radial dependence, we can express the force 
\begin{gather}
...
The amount of time steps for this stage have been established as a rule of thumb.
...
Panel a of Figure~\ref{fig:flipPatches} shows the starting configuration of a shear deformation.
Panel b of the same image pictures the deformation at a strain of $0.5L$.
For computational stability, LAMMPS flips the deformed box to a box with strain of $-0.5\cdot L$, as illustrated in panel c.
...
Although difficult to detect, we can see in panel d that the bonds align parallel to the $x$ coordinate following shear.
Now that the configurational outcomes of the patchy particle network have been briefly presented, we can continue on to the strain-stress relationships of the deformations.
...
This was proved by analyzing the system's birefringence, as well as the transient behaviour of the order tensor of entanglement strands along the chains, as described in the tube theory for entangled polymers.
...
It is also worth noting that the peak strain value and peak height increase in proportion to the shear rate.
 ...
 However, it should be noted that in figure~\ref{fig:stress-strainCLResults}, the greatest stress levels occur before the strain value of $2$, which differs with the earlier cites, which reported that the highest stress occurs at strain values of about $2-3$.
